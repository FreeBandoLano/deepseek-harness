...
只有 pi-ai（OpenAI 兼容）路由会携带它。DeepSeek 适配器有自己的请求翻译并忽略该字段，因此结构上无法承载该声明的路由目前不会被拒绝；在最早可知点拒绝它的改动另行跟踪。在该改动落地之前，位于没有该字段的路由上的 `toolChoice` 是静默不被强制的。网关接受该字段却无视它，同理无法从这里察觉：该要求只是一次请求，而“在有要求生效时以文字作答即判该轮失败”的应答半边也另行跟踪。
