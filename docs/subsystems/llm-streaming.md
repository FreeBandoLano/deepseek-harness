...
- **A compelled turn that answers in prose is an error, and a terminal one.** A `stop` that carries content but no tool call while the request declared a tool choice maps to `finish {kind:'error'}` with the `TOOL_CHOICE_UNMET` code, which the default retryable codes exclude: repeating the request would ask a model that just ignored the requirement to ignore it again. The check reads the `stop` reason only, so a response the output cap cut off still reports `max-tokens` and a completion that produced nothing at all still reports `EMPTY_RESPONSE`; see [a compelled turn fails loudly](../../.agents/notes/implemented/architecture/2026-09-13-tool-choice-enforcement.md).
